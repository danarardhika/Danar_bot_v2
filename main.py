...
"""
Bot Saham Danar v2.4 - Dengan Candlestick Chart & Screener V3
"""
...
if watchlist_str:
    ...
    WATCHLIST = ['BBCA', 'BBRI', 'BMRI', 'TLKM', 'ASII', 'UNVR', 'GOTO', 'BUMI', 'ADRO', 'ANTM', 'INCO', 'HRUM']

VOLUME_THRESHOLD = float(os.getenv('VOLUME_THRESHOLD', '1.5'))

# Cache duration (default 5 menit)
...
class Screener:
    ...
    def screen_v3(self, symbols=None):
        """
        Screener V3 dengan kriteria:
        1. Harga hari ini > MA10
        2. MA5 > MA10
        3. MA10 > MA20
        4. RSI(14) > 50 dan RSI(14) < 75
        5. Momentum(10) > 0
        6. ADX(14) > 20
        7. Volume > Volume MA20
        8. Value > 50.000.000.000
        """
        if symbols is None:
...
# ============================================
# CHART GENERATOR - DENGAN CANDLESTICK
# ============================================
...
class ChartGenerator:
    ...
    def _create_realistic_data(self, symbol, period='3mo'):
        try:
            data = REALISTIC_DATA.get(symbol)
            if not data:
                data = {'price': 5000, 'change': 0, 'volume': 10000000, 'rsi': 50, 'ma20': 5000, 'ma50': 5000, 'high': 5100, 'low': 4900}
            ...
            for i in range(n_days):
                ...
                high_price = max(open_price, price) * random.uniform(1.01, 1.03)
                low_price = min(open_price, price) * random.uniform(0.97, 0.99)
            ...
            volumes = [data['volume'] * random.uniform(0.5, 1.8) for _ in range(n_days)]
            ...
            logger.error(f"Error creating realistic data for chart: {str(e)}")
            return None
        
    def create_candlestick_chart(self, symbol: str, period: str = '3mo') -> str:
        """
        Membuat candlestick chart dengan indikator:
        - MA5, MA10, MA20, MA50
        - Volume
        - RSI
        """
        try:
            ...
            if data.empty or len(data) < 30:
                logger.warning(f"⚠️ Data kosong/tidak cukup untuk {symbol}, menggunakan fallback")
                data = self._create_realistic_data(symbol, period='3mo')
                if data is None or data.empty:
                    logger.error(f"❌ Fallback data juga kosong untuk {symbol}")
                    return None
                logger.info(f"✅ Fallback data berhasil dibuat untuk {symbol}, length: {len(data)}")
            
            # Buat figure dengan 2 subplot
            fig = plt.figure(figsize=(14, 10))
            fig.patch.set_facecolor('#0d1117')
            
            # Grid: harga (3/4) dan volume (1/4)
            ...
            # Konversi data untuk candlestick
            ...
            ma_dates = mdates.date2num(ma5.index)
            ax1.plot(ma_dates, ma5, color='#ffd93d', linewidth=1.2, alpha=0.8, label='MA5')
            ax1.plot(ma_dates, ma10, color='#ff6b6b', linewidth=1.2, alpha=0.8, label='MA10')
            ax1.plot(ma_dates, ma20, color='#6c5ce7', linewidth=1.2, alpha=0.8, label='MA20')
            ax1.plot(ma_dates, ma50, color='#00d4ff', linewidth=1.2, alpha=0.8, label='MA50')
            ...
            ax1.fill_between(ma_dates, bb_upper, bb_lower, alpha=0.1, color='#6c5ce7', label='Bollinger (20,2)')
            ax1.plot(ma_dates, bb_upper, color='#6c5ce7', linewidth=0.8, alpha=0.4, linestyle=':')
            ax1.plot(ma_dates, bb_lower, color='#6c5ce7', linewidth=0.8, alpha=0.4, linestyle=':')
            
            # Info harga terakhir            last = data.iloc[-1]
            info_text = f"O: {last['Open']:.0f} | H: {last['High']:.0f} | L: {last['Low']:.0f} | C: {last['Close']:.0f}"
            ax1.text(0.02, 0.98, info_text, transform=ax1.transAxes, fontsize=11, color='#ffffff',
            ...
            color_change = '#00d4ff' if change >= 0 else '#ff6b6b'
            
            ax1.set_title(
                f'{symbol} - {COMPANY_NAMES.get(symbol, symbol)} | {change_text}',
                color='#ffffff', 
                fontsize=14, 
                fontweight='bold', 
                pad=20
            )
            ax1.set_ylabel('Harga', color='#a0aec0', fontsize=11)
            ax1.grid(True, alpha=0.15, color='#2d3748')
            ax1.legend(loc='upper left', facecolor='#1f2937', edgecolor='#2d3748', labelcolor='#ffffff', fontsize=9)
            ...
            # Warna volume: hijau jika close > open, merah jika close < open
            colors_vol = ['#00d4ff' if data['Close'].iloc[i] >= data['Open'].iloc[i] else '#ff6b6b' for i in range(len(data))]
            ax2.bar(data.index, data['Volume'], color=colors_vol, alpha=0.6, width=0.8)
            
            # MA Volume
            ma_vol = data['Volume'].rolling(20).mean()
            ax2.plot(data.index, ma_vol, color='#ffd93d', linewidth=1.5, alpha=0.7, label='MA Volume 20')
            
            # RSI di volume plot (sebagai garis kecil)
            rsi = self._calculate_rsi(data)
            rsi_scaled = (rsi / 100) * data['Volume'].max() * 0.3
            ax2.plot(data.index, rsi_scaled, color='#6c5ce7', linewidth=1, alpha=0.6, label='RSI (14)')
            
            ax2.set_ylabel('Volume', color='#a0aec0', fontsize=11)
            ax2.grid(True, alpha=0.15, color='#2d3748')
            ax2.legend(loc='upper left', facecolor='#1f2937', edgecolor='#2d3748', labelcolor='#ffffff', fontsize=9)
            ax2.xaxis.set_major_formatter(mdates.DateFormatter('%b %d'))
            ax2.xaxis.set_major_locator(mdates.WeekdayLocator(interval=2))
            ax2.tick_params(colors='#a0aec0')
            
            # Footer
            footer_text = f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Data: {'Real-time' if not self._using_fallback else 'Realistis'}"
    ...
    def _using_fallback(self):
        """Cek apakah menggunakan fallback data"""
        return False  # akan di-set dari bot
...
class SahamBot:
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Help lengkap dengan SEMUA perintah"""
        help_text = (
    ...
    async def screener_v3(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Screener V3 dengan kriteria advanced"""
    ...
    async def refresh_price(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Refresh harga terbaru dari Yahoo Finance (skip cache)"""
        if not context.args:
            await update.message.reply_text(
                "❌ Gunakan: /rp SYMBOL\n\n"
                "Contoh: /rp BBCA\n"
    ...
    async def force_refresh_all(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Force refresh semua data di watchlist"""
    ...
    # ============================================
    # WATCHLIST - SUDAH DIPERBAIKI
    # ============================================

    async def watchlist(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Menampilkan semua saham di watchlist dengan harga terbaru"""
    ...
    async def chart_custom(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Chart custom dengan CANDLESTICK
        Mendukung alias: /cc, /chart_custom, /chartcustom, /customchart
        """
        if not context.args:
            await update.message.reply_text(
                "❌ Gunakan: /cc SYMBOL\n\n"
                ...
                "/cc BUMI - Candlestick chart BUMI\n"
                "/cc ADRO - Candlestick chart ADRO\n\n"
                "📊 *Indikator:*\n"
                "• Candlestick OHLC\n"
                "• MA5, MA10, MA20, MA50\n"
                "• Bollinger Bands (20,2)\n"
                "• Volume dengan MA Volume 20\n"
                "• RSI (14) overlay di volume\n\n"
                "📌 Bisa chart SEMUA saham!",
                parse_mode='Markdown'
            )
            return
        ...
        try:
            ...
            if chart_path and os.path.exists(chart_path):
                ...
                with open(chart_path, 'rb') as photo:
                    await update.message.reply_photo(
                        ...
                        caption=f"📈 *{symbol} - Candlestick Chart*\n"
                               f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                               f"*Indikator:* MA5, MA10, MA20, MA50, Bollinger, RSI\n"
                ...
                await msg.edit_text(
                    ...
                    f"• Data tidak cukup (minimal 30 hari)\n"
    ...
    def run(self):
        try:
            ...
            logger.info(f"📌 Screener V3: Aktif")
            logger.info(f"📌 Chart: Candlestick Chart (MA5, MA10, MA20, MA50, Bollinger, RSI)")
            logger.info(f"📌 Alias: /cc, /chart_custom, /chartcustom, /customchart")
            logger.info(f"📌 Refresh: /rp SYMBOL, /force_refresh")
            ...
            # Print banner
            print("=" * 60)
            print("🤖 BOT SAHAM DANAR v2.4 - CANDLESTICK CHART")
            print("=" * 60)
            print(f"📌 Token: ✓")
            print(f"📌 Watchlist: {len(self.watchlist)} saham")
            print(f"📌 Cache Duration: {CACHE_DURATION_MINUTES} menit")
            print(f"📌 Screener V3: Aktif (Advanced Screening)")
            print(f"📌 Chart: Candlestick + MA5, MA10, MA20, MA50, Bollinger, RSI")
            print(f"📌 Alias: /cc, /chart_custom, /chartcustom, /customchart")
            print(f"📌 Refresh: /rp SYMBOL, /force_refresh")
            print(f"📌 Environment: {ENVIRONMENT}")
            print("=" * 60)
            print("📊 Bot siap menerima perintah!")
            print("💡 Contoh: /screener_v3, /rp BBCA, /cc BUMI, /watchlist")
            print("=" * 60)

            # Run bot
            if ENVIRONMENT == 'production':
                logger.info("🚀 Running in PRODUCTION mode with webhook")
                print("🌐 Running with webhook on port", PORT)
                
                application.run_webhook(
                    listen="0.0.0.0",
                    port=PORT,
                    url_path=TELEGRAM_TOKEN,
                    webhook_url=f"https://your-app-name.railway.app/{TELEGRAM_TOKEN}"
                )
            else:
                logger.info("🚀 Running in DEVELOPMENT mode with polling")
                print("📡 Running with polling (development mode)")
