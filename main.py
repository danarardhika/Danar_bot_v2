...
"""
Bot Saham Danar v2.4 - Dengan Candlestick Chart (Manual) & Screener V3
"""
...
RAILWAY_URL = os.getenv('RAILWAY_URL', 'https://your-app-name.railway.app')
...
# ============================================
# CHART GENERATOR - CANDLESTICK MANUAL
# ============================================
...
class ChartGenerator:
    ...
    def draw_candlestick(self, ax, data, width=0.6):
        """Manual candlestick drawing tanpa mplfinance"""
        for i, (idx, row) in enumerate(data.iterrows()):
            ...
            # Warna: hijau jika close > open
            color = '#00d4ff' if close_price >= open_price else '#ff6b6b'
            
            # Body candlestick
            body_bottom = min(open_price, close_price)
            body_top = max(open_price, close_price)
            body_height = max(body_top - body_bottom, 0.01)
            
            # Gambar body (persegi panjang)
            rect = Rectangle(
            ...
            # Gambar upper shadow (wick)
            ax.plot([idx, idx], [body_top, high_price], color=color, linewidth=1.5)
            
            # Gambar lower shadow (wick)
            ax.plot([idx, idx], [body_bottom, low_price], color=color, linewidth=1.5)
    ...
    def create_candlestick_chart(self, symbol: str, period: str = '3mo') -> str:
        try:
            ...
            self.draw_candlestick(ax1, data, width=0.6)
            ...
            ax1.plot(x_values, ma5, color='#ffd93d', linewidth=1.2, alpha=0.8, label='MA5')
            ax1.plot(x_values, ma10, color='#ff6b6b', linewidth=1.2, alpha=0.8, label='MA10')
            ax1.plot(x_values, ma20, color='#6c5ce7', linewidth=1.2, alpha=0.8, label='MA20')
            ax1.plot(x_values, ma50, color='#00d4ff', linewidth=1.2, alpha=0.8, label='MA50')
            ...
            ax1.fill_between(x_values, bb_upper, bb_lower, alpha=0.1, color='#6c5ce7')
            ax1.plot(x_values, bb_upper, color='#6c5ce7', linewidth=0.8, alpha=0.4, linestyle=':')
            ax1.plot(x_values, bb_lower, color='#6c5ce7', linewidth=0.8, alpha=0.4, linestyle=':')
            ...
            tick_labels = [data.index[i].strftime('%b %d') for i in tick_positions]
            ax1.set_xticks(tick_positions)
            ax1.set_xticklabels(tick_labels, color='#a0aec0')
            ...
            ax2.bar(x_values, data['Volume'], color=colors_vol, alpha=0.6, width=0.8)
            
            # MA Volume
            ma_vol = data['Volume'].rolling(20).mean()
            ax2.plot(x_values, ma_vol, color='#ffd93d', linewidth=1.5, alpha=0.7, label='MA Volume 20')
            ...
            ax2.set_xticklabels(tick_labels, color='#a0aec0')
            ax2.tick_params(colors='#a0aec0')
            
            # Footer
            footer_text = f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Data: {'Real-time' if not self.screener.use_fallback else 'Realistis'}"
...
class SahamBot:
                ...
                with open(chart_path, 'rb') as photo:
                    await update.message.reply_photo(
                        ...
                        caption=f"📈 *{symbol} - Candlestick Chart*\n"
                               f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                               f"*Indikator:* MA5, MA10, MA20, MA50, Bollinger\n"
    ...
    def run(self):
        try:
            ...
            print(f"📌 Chart: Candlestick (Manual) + MA5, MA10, MA20, MA50, Bollinger")
            ...
            if ENVIRONMENT == 'production':
                logger.info("🚀 Running in PRODUCTION mode with polling (webhook disabled)")
                print("📡 Running with polling (recommended for Railway)")
