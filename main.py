...
"""
Bot Saham Danar v2.4 - Dengan Alias Command (FIXED - NO WATCHLIST RESTRICTION)
"""
...
class Screener:
    ...
    def _create_realistic_data(self, symbol, period='1mo'):
        try:
            ...
            if not data:
                # Default jika simbol tidak ada di realistic data
...
class ChartGenerator:
    ...
    def _create_realistic_data(self, symbol, period='3mo'):
        """Buat data realistis untuk fallback"""
        try:
            data = REALISTIC_DATA.get(symbol)
            if not data:
                # Default jika simbol tidak dikenal
                data = {'price': 5000, 'change': 0, 'volume': 10000000, 'rsi': 50, 'ma20': 5000, 'ma50': 5000, 'high': 5100, 'low': 4900}
    ...
    def create_chart(self, symbol: str, period: str = '3mo') -> str:
        try:
            ...
            # FALLBACK: Jika data kosong atau tidak cukup
...
class SahamBot:
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            ...
            "/chart_custom SYMBOL - Chart lengkap (4 indikator)\n"
            "/chartcustom SYMBOL - Sama seperti di atas\n"
            "/customchart SYMBOL - Sama seperti di atas\n"
            "/cc SYMBOL - Singkatan chart custom ⭐\n\n"
            "*📌 Contoh:*\n"
            "/cc BBCA - Chart BBCA\n"
            "/cc BUMI - Chart BUMI\n"
            "/cc ADRO - Chart ADRO\n\n"
            ...
            "*⚠️ Catatan:*\n"
            "• Chart custom: Bollinger, SMA, MACD, RSI, Stochastic\n"
            "• Bisa chart SEMUA saham (BBCA, BUMI, ADRO, dll)\n"
            "• Gunakan /refresh jika data tidak muncul",
            parse_mode='Markdown'
        )
    ...
    async def chart_custom(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Handler untuk chart custom - TANPA BATASAN WATCHLIST!
        Bisa chart SEMUA saham (BBCA, BUMI, ADRO, dll)
        """
        if not context.args:
            await update.message.reply_text(
        ...
        # ⚠️ HAPUS VALIDASI WATCHLIST - SEKARANG BISA CHART SEMUA SAHAM!
        # Langsung proses chart tanpa membatasi simbol
        ...
        try:
            ...
            if chart_path and os.path.exists(chart_path):
                ...
                await msg.edit_text(
                    ...
                    f"• Simbol tidak valid (cek di Yahoo Finance)\n\n"
                    f"💡 *Solusi:*\n"
    ...
    async def _error_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if update and update.effective_message:
                error_msg = str(context.error)[:150]
    ...
    def run(self):
        try:
            ...
            print("=" * 60)
            print("🤖 BOT SAHAM DANAR v2.4 - NO WATCHLIST RESTRICTION")
            print("=" * 60)
            print(f"📌 Token: ✓")
            print(f"📌 Watchlist: {len(self.watchlist)} saham (untuk monitoring)")
            ...
            print("💡 Contoh: /cc BUMI, /cc ADRO, /cc BBCA")
            print("=" * 60)

            # Run bot sesuai environment
            if ENVIRONMENT == 'production':
                logger.info("🚀 Running in PRODUCTION mode with webhook")
                print("🌐 Running with webhook on port", PORT)
                
                # Gunakan webhook untuk production (Railway)
