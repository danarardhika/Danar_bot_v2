...
"""
Bot Saham Danar v2.4 - Dengan Alias Command (FIXED VERSION)
"""
...
class SahamBot:
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            ...
            "• Format simbol: BBCA, BBRI, TLKM, dll",
            parse_mode='Markdown'
        )
    ...
    async def chart_custom(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handler untuk chart custom - mendukung berbagai alias"""
        if not context.args:
            await update.message.reply_text(
                "❌ *Gunakan:* /cc SYMBOL\n\n"
                "📌 *Contoh:* /cc BBCA\n\n"
        ...
        # Validasi simbol
        if symbol not in self.watchlist and symbol not in COMPANY_NAMES:
            await update.message.reply_text(
                f"❌ Simbol *{symbol}* tidak dikenal.\n\n"
                f"📋 *Watchlist saat ini:*\n"
                f"{', '.join(self.watchlist[:10])}{'...' if len(self.watchlist) > 10 else ''}\n\n"
                f"Gunakan /watchlist untuk melihat semua.",
                parse_mode='Markdown'
            )
            return
    ...
    def run(self):
        try:
            ...
            logger.info(f"📌 Chart Custom: Aktif")
            ...
            print("=" * 60)
            print("🤖 BOT SAHAM DANAR v2.4 - FIXED VERSION")
            print("=" * 60)
            print(f"📌 Token: ✓")
