...
"""
Bot Saham Danar v2.4 - Dengan Screener V3 (Advanced Screening)
"""
...
class Screener:
    ...
    def screen_v3(self, symbols=None):
        ...
        for symbol in symbols:
            try:
                total_checked += 1
                data = self.get_stock_data(symbol, period='2mo')  # butuh 60 hari untuk ADX
                ...
                # Kriteria V3
                criteria = {
                    'price_gt_ma10': ind['price'] > ind['ma10'],
                    'ma5_gt_ma10': ind['ma5'] > ind['ma10'],
                    'ma10_gt_ma20': ind['ma10'] > ind['ma20'],
                    'rsi_range': 50 < ind['rsi'] < 75,
                    'momentum_gt_0': ind['momentum'] > 0,
                    'adx_gt_20': ind['adx'] > 20,
                    'volume_gt_ma20': ind['volume'] > ind['ma20_volume'],
                    'value_gt_50b': ind['value'] > 50000000000  # 50 Milyar
                }
...
class ChartGenerator:
    ...
    def _create_realistic_data(self, symbol, period='3mo'):
        try:
            ...
            for i in range(n_days):
                ...
                if i == 0:
                    prices.append(base_price * random.uniform(0.95, 1.05))
                else:
                    prices.append(prices[-1] * (1 + change))
    ...
    def create_chart(self, symbol: str, period: str = '3mo') -> str:
        try:
            ...
            ax1.set_title(f'{symbol} - {COMPANY_NAMES.get(symbol, symbol)} Custom Chart', color='#ffffff', fontsize=14, fontweight='bold', pad=20)
            ...
            logger.info(f"✅ Chart custom {symbol} berhasil dibuat: {filename}")
            return filename
...
class SahamBot:
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        help_text = (
            ...
            "📈 Chart:\n"
            "/chart SYMBOL - Chart sederhana (2 panel)\n"
            "/cc SYMBOL - Chart custom lengkap (4 indikator) ⭐\n\n"
    ...
    async def screener_v3(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for i, stock in enumerate(result['results'][:10], 1):
                ...
                criteria_passed = []
                for key, val in stock['criteria'].items():
                    if val:
                        criteria_passed.append(key.replace('_', ' ').title())
    ...
    async def chart_custom(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:
            await update.message.reply_text(
                ...
                "Catatan: Bisa chart SEMUA saham!"
            )
            return
        ...
        msg = await update.message.reply_text(
            f"📊 Membuat chart custom {symbol}...\n"
            f"⏳ Mohon tunggu (10-15 detik)..."
        )
        
        try:
            logger.info(f"📊 Generating custom chart for {symbol}")
            chart_path = self.chart_gen.create_chart(symbol, period='3mo')
            ...
            if chart_path and os.path.exists(chart_path):
                ...
                with open(chart_path, 'rb') as photo:
                    await update.message.reply_photo(
                        ...
                        caption=f"📈 {symbol} - Custom Chart\n"
                               f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                               f"Indikator: Bollinger, SMA, MACD, RSI, Stochastic\n"
                ...
                await msg.edit_text(
                    ...
                    f"- Coba /chart {symbol} (chart sederhana)"
                )
        except Exception as e:
    ...
    def run(self):
        try:
            ...
            logger.info(f"📌 Chart Custom: Aktif (TANPA BATASAN WATCHLIST)")
            ...
            application.add_handler(CommandHandler("screener_v3", self.screener_v3))  # NEW!
            ...
            # Chart custom dengan banyak alias
            ...
            # Print banner
            print("=" * 60)
            print("🤖 BOT SAHAM DANAR v2.4 - DENGAN SCREENER V3")
            print("=" * 60)
            print(f"📌 Token: ✓")
            ...
            print(f"📌 Chart Custom: Bisa chart SEMUA saham!")
            print(f"📌 Alias: /chart_custom, /chartcustom, /customchart, /cc")
            print(f"📌 Refresh: /rp SYMBOL, /force_refresh")
            print(f"📌 Environment: {ENVIRONMENT}")
            print("=" * 60)
            print("📊 Bot siap menerima perintah!")
            print("💡 Contoh: /screener_v3, /rp BBCA, /cc BUMI")
            print("=" * 60)
